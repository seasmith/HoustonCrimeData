...
hp_beats <- st_make_valid(hp_beats)  # apparently 05F40 is in valid (vertex dupe)
...
hou_pop <- hou_pop %>%
 mutate(!!!fill_gaps(2010, 2015, UN_2005_E, UN_2010_E),
        ...
        !!!fill_gaps(2020, 2025, UN_2015_E, UN_2020_E))
...
hou_pop <- hou_pop %>%
 inner_join(select(as_tibble(hp_beats), Beats, Area_sq_mi), by = "Beats")
